# Susannah Weatherby CS499 Milestone Three Project
...
class Appointment:
    ...
    def format(self, contact_service):        
        ...
        assigned_info = f"{self.assigned_customer_id}, ({name})" if name else self.assigned_customer_id

        return (f"[Appointment] ID: {self.id}, Date: {self.appointment_date.strftime('%Y-%m-%d %I:%M %p')}, "
                f"Desc: {self.description}, Customer: ID {assigned_info}")
...
class Task:
    ...
    def format(self, contact_service):
        ...
        assigned_info = f"{self.assigned_employee_id}, ({name})" if name else self.assigned_employee_id
        
        return (f"[Task] ID: {self.id}, Name: {self.name}, Description: {self.desc}, Assigned Employee: ID {assigned_info}")
    ...
    def delete_appointment(self, id):
        for i, appt in enumerate(self.appointments):
            if appt.get_id() == id:
                ...
                print(f"Task for {appt.assigned_employee_id} deleted.")
                return
